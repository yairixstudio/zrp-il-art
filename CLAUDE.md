...
| `/works/` | The Art Works | `XhGH...::542:499` (canonical 2026-06-10; layout+content) — legacy `491:928` | `XhGH...::489:4` (older content source) | ✅ `works/index.html` — **גריד 3 עמודות דסקטופ / 2 מובייל** (עודכן 2026-06-10), כרטיסי יצירה (אומן/כותרת/גלריה/נמכר/פרטים, `white-space:pre-line` לפרטים רב-שורתיים), לייטבוקס single. Data ב-`works.json` `art_works[]` = **102 יצירות**, סדר **1:1 מול Figma `542:500` (Frame 419, הגריד בתוך `542:499`)** — אומת מיקום-מיקום 2026-06-10 (⚠️ הפריים `mode:none` = absolute; סדר ויזואלי = מיון לפי y,x — סדר הילדים במקרה זהה). תמונות ב-`images/works/v2/`. **עדכון Figma 2026-06-10:** 7 יצירות nir-giorgio-levin (`-3..9`) הועברו ל**ראש הגריד** (nodes `583:67..131`), `yarden-amir-3` (ציפורי החופש) עבר לאחרי `nir-giorgio-levin-2`, ונוספה **racheli-reuven-1** ("עשי בי קרבה", פרטים "שיר", דיזנגוף, node `575:7527`, imageRef `157b18b5`) בין `sami-david-4` ל-`zohar-ron-1` — היצירה הראשונה של רחלי ראובן (עדיין דף placeholder). כתיב שמות בעברית: ה-JSON שומר על הכתיב הקנוני של `artists.json` גם כשה-Figma שונה (נעמי/נועמי, קינן/כנען, ליוואי/ליביי, נטאשה/נטשה, ריסה ברוק עוז/ריסה עוז, ליאל סלמן נעם/ליאל סלמן). legacy: `491:928`, `Zn3N...::1213:2615` |
