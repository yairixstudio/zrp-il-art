...
### 1.4.1 Homepage — tribe teaser (מובייל, מעל `#press`)

- **מדיה קנונית — Figma `250:98`** (רצועת 4 תמונות; ראה `FIGMA_LINKS.md`).
- המוזאות **אינן** קישורי ניווט: `div.tribe-thumb-card` עם `data-artwork-src` + `data-artwork-title` בלבד; **ללא** `data-artwork-link` ⇒ אין כפתור CTA בלייטבוקס.
- עוטפים את המסילה ב-`data-artwork-gallery="tribe-teaser"` ל-prev/next בתוך הרצועה.
- מעבר לדף התערוכה נשאר דרך `.tribe-teaser-head` בלבד (`exhibition_href` ב-`homepage.json`).
